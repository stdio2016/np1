...
int tryToGetAck(int lv, char type) {
  while (1) {
...
void send_file(char *filename) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (writeIsPossible(38)) {
      send(sockfd, msg, n + 13, 0);
    }
    int y = tryToGetAck(38, SILLY_INIT_ACK);
    if (y == 0) {
      break;
    }
  }
  ...
  while (!feof(fileToSend)) {
    ...
    for (i = 10; i < 40; i++) {
      if (writeIsPossible(i)) {
        send(sockfd, msg, n + 12, 0);
      }
      currentSN += n;
      int y = tryToGetAck(i, SILLY_ACK);
      if (y == 0) {
        break;
      }
      currentSN -= n;
  ...
  for (i = 10; i < 40; i++) {
    ...
    int y = tryToGetAck(i, SILLY_STOP_ACK);
    if (y == 0) {
      break;
    }
  }
